...
def gen_output_filename(booktitle, file, template):
    """Generates an output filename based on different attributes of the file"""
...
def combine_audiofiles(filenames, tmp_dir, output_path):
    ...
    with open(combine_file, "a") as f:
        for i in filenames:
            f.write(f"file '{i}'\n")
    subprocess.run(["ffmpeg", "-f", "concat", "-safe", "0", "-i", combine_file, "-c", "copy", output_path], capture_output=True)
